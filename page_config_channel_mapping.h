...
void send_config_channel_mapping_vals(AsyncWebServerRequest *request) {
  // TODO: Switch to next universe if more than 512 channels are required
    ...
    values += "mapping|div|<table border='0' cellspacing='0' cellpadding='3'><tr bgcolor='#DDDDDD' ><td><strong>Ch</strong></td><td><strong>Output port #</strong></td></tr>";
    for (int i = 0; i < config.channel_count; i++) {
        values += "<tr><td>" + String(i + 1) + "</td><td>-> <input type='text' name='map" + String(i+1) + "' value='" + String(mapping[i]) + "'></td></tr>";
    }
    values += "</table>\n";
